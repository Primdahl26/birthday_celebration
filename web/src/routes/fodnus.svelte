	...
	<div class="overflow-x-auto shadow-lg py-5">
		<table class="table table-zebra w-full">
			<thead>
				<tr>
					<td>
						{#if orders.filter((val) => val.completed == false).length}
							<div class="flex flex-row gap-2 place-items-center">
								<button
									on:click={() => {
										const confirmed = confirm(
											`Bekræft! - Er du sikker på du vil slette alle ${
												orders.filter((val) => val.completed == false).length
											} ordre?`
										)
										if (confirmed) {
								...
								<h1 class="text-xl">{orders.filter((val) => val.completed == false).length}</h1>
							</div>
						{:else}
							<div class="flex flex-row gap-2 place-items-center">
